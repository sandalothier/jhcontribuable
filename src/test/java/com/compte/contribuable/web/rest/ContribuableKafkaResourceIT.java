package com.compte.contribuable.web.rest;

import com.compte.contribuable.config.KafkaProperties;
...
class ContribuableKafkaResourceIT {
    ...
    void setup() {
        ...
        ContribuableKafkaResource kafkaResource = new ContribuableKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/contribuable-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/contribuable-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
